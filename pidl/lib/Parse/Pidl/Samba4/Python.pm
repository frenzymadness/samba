...
sub PythonType($$$$)
{
	...
	if ($actual_ctype->{TYPE} eq "UNION" and defined($actual_ctype->{ELEMENTS})) {
		...
		$self->deindent;
		$self->pidl("}");
		$self->pidl("result = PyCapsule_New(out, NULL, NULL);");
		$self->pidl("PyCapsule_SetContext(result, discard_const_p(char, out_type));");
		$self->pidl("return result;");
		$self->deindent;
		$self->pidl("}");
		$self->pidl("");
...
");
	...
	# $self->pidl_hdr("MODULE_INIT_FUNC($basename)");
