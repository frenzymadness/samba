...
static PyObject *py_GUID_richcmp(PyObject *py_self, PyObject *py_other, int op)
{
	...
	switch (op) {
		case Py_EQ: return ret == 0 ? Py_True : Py_False;
		case Py_NE: return ret != 0 ? Py_True : Py_False;
		case Py_LT: return ret <  0 ? Py_True : Py_False;
		case Py_GT: return ret >  0 ? Py_True : Py_False;
		case Py_LE: return ret <= 0 ? Py_True : Py_False;
		case Py_GE: return ret >= 0 ? Py_True : Py_False;
	}
	Py_INCREF(Py_NotImplemented);
	return Py_NotImplemented;
}
