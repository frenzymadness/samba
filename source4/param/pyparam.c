...
static PyObject *py_lp_service_dump(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "O|b", &py_default_service, &show_defaults)) {
		return NULL;
	}
	...
	lpcfg_dump_one(stdout, show_defaults, service, default_service);
...
static PyMethodDef py_lp_service_methods[] = {
	{ "dump", (PyCFunction)py_lp_service_dump, METH_VARARGS, 
		"S.dump(default_service, show_defaults=False)" },
	{ NULL }
};
